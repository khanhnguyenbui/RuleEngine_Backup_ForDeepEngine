...
class ExecutePersonalization(Resource):
    ...
    @classmethod
    def make_api(cls, es,_index_per,_index,_index_query):
        cls.es = es
        cls._index_per = _index_per
        cls._index = _index
    ...
    def get(self, userID, perID):
        ...
        for p in per:
            queryID = p['queryID']
            point = p['point']
            totalPoint = totalPoint + self.calPoint(userID, queryID, point)
        return totalPoint
    
    def calPoint(self,userID, queryID, point):
        res = Utils.exeQuery(queryID, self.es,self._index, self._index_query)
        if len(res) == 0:
            return 0
        listUsers = list()
        for user in res:
            print(user)
            listUsers.append(user['_id'])
